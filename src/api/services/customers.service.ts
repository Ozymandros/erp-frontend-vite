...
class CustomersService {
  ...
  async searchCustomers(querySpec: QuerySpec): Promise<PaginatedResponse<CustomerDto>> {
    return this.apiClient.get<PaginatedResponse<CustomerDto>>("/sales/api/sales/customers/search", {
      params: querySpec
    })
  }

  async createCustomer(data: CustomerDto): Promise<CustomerDto> {
  ...
  async updateCustomer(id: string, data: CustomerDto): Promise<CustomerDto> {
    return this.apiClient.put<CustomerDto>(`/sales/api/sales/customers/${id}`, data)
  }
